...
class AppDatabase {
  ...
  Future<void> _seedCategories(Database db) async {
    final categories = [
      const Category(name: 'Comida', color: 0xFFE76F51, icon: 'restaurant'),
      const Category(name: 'Transporte', color: 0xFF457B9D, icon: 'commute'),
      const Category(name: 'Vivienda', color: 0xFF2A9D8F, icon: 'home'),
      const Category(name: 'Servicios', color: 0xFFE9C46A, icon: 'bolt'),
      const Category(name: 'Salud', color: 0xFF8AB17D, icon: 'health'),
      const Category(name: 'Ocio', color: 0xFF9B5DE5, icon: 'movie'),
      const Category(name: 'Otros', color: 0xFF6C757D, icon: 'more'),
    ];
  ...
  Future<List<SavingsAccount>> getAccounts() async {
    final db = await database;
    final maps = await db.query('ahorros_inversiones', orderBy: 'created_at DESC');
